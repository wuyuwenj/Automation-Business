"""
A2A server entry point for the agent evaluator.

Runs as an A2A-compliant agent with payment validation middleware.

Usage:
    poetry run agent-a2a
    poetry run agent-a2a --tools check --port 9031
    poetry run agent-a2a --tools discover,evaluate --port 9032
"""
...
class StrandsA2AExecutor(AgentExecutor):
    ...
    def __init__(
        ...
        self._log = get_logger("evaluator.executor")
...
def _register_with_buyer(buyer_url: str, agent_url: str):
    ...
    _reg_log = get_logger("evaluator.register")
...
def _parse_args():
    parser = argparse.ArgumentParser(description="Agent Evaluator -- A2A Mode")
    parser.add_argument(
        "--tools",
        nargs="*",
        choices=list(ALL_TOOLS.keys()),
        default=None,
        help="Tools to expose (default: all). Options: check, discover, evaluate",
    )
    parser.add_argument(
        "--port",
        type=int,
        default=int(os.getenv("A2A_PORT", os.getenv("PORT", "9030"))),
        help="Port to listen on (default: A2A_PORT or PORT env, or 9030)",
    )
    parser.add_argument(
        "--buyer-url",
...
def main():
    ...
    base_agent_card = {
        ...
        "description": (
            "Agent QA and health monitoring service. Checks availability, "
            "discovers agents via the hackathon Discovery API, and produces "
            "ranked evaluation reports."
        ),
    ...
    log(_logger, "SERVER", "STARTUP",
        f"Agent Evaluator -- A2A Mode on port {port}")
    log(_logger, "SERVER", "STARTUP",

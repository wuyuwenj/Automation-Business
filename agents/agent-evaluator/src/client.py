"""
HTTP x402 client - demonstrates the payment flow for the agent evaluator.
...
SERVER_URL = os.getenv("SERVER_URL", "http://localhost:3030")
...
def main():
    print("=" * 60)
    print("x402 Payment Flow - Agent Evaluator")
    print("=" * 60)
    print(f"\nServer: {SERVER_URL}")
    ...
    with httpx.Client(timeout=60.0) as client:
        ...
        response1 = client.post(
            f"{SERVER_URL}/data",
            json={"query": "Discover all available seller agents in the marketplace"},
        )

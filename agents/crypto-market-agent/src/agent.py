...
app = FastAPI(
    ...
    description="Crypto & DeFi market data agent with x402 payment-protected tools",
)
...
async def data(request: Request, body: DataRequest) -> JSONResponse:
    """Query crypto market data through the Strands agent."""
    try:
...
def main():
    ...
    print(f"Crypto Market Agent running on http://localhost:{PORT}")
    print(f"\nPlan ID: {NVM_PLAN_ID}")
    print("\nEndpoints:")
    print("  POST /data     - Query crypto data (x402 payment required)")

"""
HTTP x402 client - demonstrates the payment flow for the crypto market agent.
...
SERVER_URL = os.getenv("SERVER_URL", "http://localhost:3010")
...
def main():
    print("=" * 60)
    print("x402 Payment Flow - Crypto Market Agent")
    print("=" * 60)
    print(f"\nServer: {SERVER_URL}")
    ...
    with httpx.Client(timeout=60.0) as client:
        ...
        response1 = client.post(
            f"{SERVER_URL}/data",
            json={"query": "What is the current price of Bitcoin and Ethereum?"},
        )

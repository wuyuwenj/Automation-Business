...
app = FastAPI(
    ...
    description="Web scraping and content extraction agent with x402 payment-protected tools",
)
...
async def data(request: Request, body: DataRequest) -> JSONResponse:
    """Query web scraping services through the Strands agent."""
    try:
...
def main():
    ...
    print(f"Web Scraper Agent running on http://localhost:{PORT}")
    print(f"\nPlan ID: {NVM_PLAN_ID}")
    print("\nEndpoints:")
    print("  POST /data     - Query scraping services (x402 payment required)")

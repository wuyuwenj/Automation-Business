"""
A2A server entry point for the web scraper agent.

Runs as an A2A-compliant agent with payment validation middleware.

Usage:
    poetry run agent-a2a
    poetry run agent-a2a --tools scrape --port 9021
    poetry run agent-a2a --tools batch,deep --port 9022
"""
...
class StrandsA2AExecutor(AgentExecutor):
    ...
    def __init__(
        ...
        self._log = get_logger("scraper.executor")
...
def _register_with_buyer(buyer_url: str, agent_url: str):
    ...
    _reg_log = get_logger("scraper.register")
...
def _parse_args():
    parser = argparse.ArgumentParser(description="Web Scraper Agent -- A2A Mode")
    parser.add_argument(
        "--tools",
        nargs="*",
        choices=list(ALL_TOOLS.keys()),
        default=None,
        help="Tools to expose (default: all). Options: scrape, batch, deep",
    )
    parser.add_argument(
        "--port",
        type=int,
        default=int(os.getenv("A2A_PORT", os.getenv("PORT", "9020"))),
        help="Port to listen on (default: A2A_PORT or PORT env, or 9020)",
    )
    parser.add_argument(
        "--buyer-url",
...
def main():
    ...
    base_agent_card = {
        ...
        "description": (
            "Web scraping and content extraction agent powered by Apify. "
            "Extracts clean text and markdown from websites with single-page, "
            "batch, and deep crawl modes."
        ),
    ...
    log(_logger, "SERVER", "STARTUP",
        f"Web Scraper Agent -- A2A Mode on port {port}")
    log(_logger, "SERVER", "STARTUP",

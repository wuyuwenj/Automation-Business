"""
A2A server entry point for the creative generation selling agent.
...
Usage:
    ...
    poetry run agent-a2a --tools copy --port 9001
    poetry run agent-a2a --tools brand --port 9002 --buyer-url http://localhost:8000
"""
...
def _parse_args():
    parser = argparse.ArgumentParser(description="Creative Generation Selling Agent — A2A Mode")
    parser.add_argument(
        "--tools",
        nargs="*",
        choices=list(ALL_TOOLS.keys()),
        default=None,
        help="Tools to expose (default: all). Options: copy, brand, landing",
    )
    parser.add_argument(
        "--port",
        type=int,
...
def main():
    ...
    log(_logger, "SERVER", "STARTUP",
        f"Creative Generation Selling Agent — A2A Mode on port {port}")
    log(_logger, "SERVER", "STARTUP",
